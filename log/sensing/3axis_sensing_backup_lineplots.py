...
BASELINE_SAMPLES = 500   # Baseline 계산에 사용할 샘플 수
NOISE_THRESHOLD_N = 0.3  # 이 값(N) 이하의 힘은 노이즈로 간주하여 0으로 처리
CROSSTALK_ACTIVATION_THRESHOLD = 20 # 한 포인트의 센서들 평균 변화량이 이 값 이상이어야 유효 터치로 간주 (튜닝 필요)
...
class RealtimeForcePlotter:
    def __init__(self):
        ...
        self.plot_data = {
            ...
            'forces': {p_i: [deque(maxlen=HISTORY_SIZE) for _ in range(3)] for p_i in range(1, 10)}
        }
        self.app = None
        self.win = None
        self.plots = {}
    ...
    def setup_plot(self):
        ...
        self.win = CustomGraphicsLayoutWidget(show=True, title="실시간 3축 힘 센싱 (P1-P9) | 'r' 키로 영점 재설정")
