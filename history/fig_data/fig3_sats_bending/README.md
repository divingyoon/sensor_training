...
| `flat_sr/d5_final/` | d5-only + β 물성보정 (0.5mm 출력) — 패널 D5 A~F + README | 완료 |
| `bending/` | 밴딩 파트 (패널 B~E) — **데이터 취득 대기**, 취득 스펙은 폴더 README | 준비 |

## 재현

- flat SR 패널: `generate_fig3_sats.py --figset {xy0p5_final|d5_final}` (진단 npz: `experiments_archive/`)
- 밴딩: `sats/bending/` (Phase 0 완료, 취득 후 P1→P4 — `bending/README.md`)

소재 비교(구 Fig3B 등)는 논문 Fig.2D로 이동 → `fig2_material_ablation/panelD_sats/`.
