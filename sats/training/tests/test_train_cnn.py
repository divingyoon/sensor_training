...
from sats.training.train_cnn import (
    load_local_map_weights,
    train_epoch,
    val_epoch,
)
...
def _make_cnn_stage(cfg=None):
    cfg = cfg or _small_cfg()
    return SATSCNNStage(cfg)


def _save_local_map_ckpt(cfg, path: Path):
    """SATSLocalMapStage 가중치를 체크포인트 형식으로 저장한다."""
    lm_stage = SATSLocalMapStage(cfg)
    torch.save(
        {
            "epoch": 1,
            "model": lm_stage.state_dict(),
            "metrics": {},
        },
        path,
    )
...
# ─────────────────────────────────────────────────────────────────────────────
# load_local_map_weights 테스트
# ─────────────────────────────────────────────────────────────────────────────

class TestLoadLocalMapWeights:

    def test_encoder_weights_loaded_correctly(self):
        """체크포인트의 encoder 가중치가 stage.encoder에 정확히 로드된다."""
        cfg = _small_cfg()
        with tempfile.NamedTemporaryFile(suffix=".pt") as f:
            lm_stage = _save_local_map_ckpt(cfg, Path(f.name))
            cnn_stage = _make_cnn_stage(cfg)

            load_local_map_weights(f.name, cnn_stage, freeze=False)

            for (n1, p1), (n2, p2) in zip(
                lm_stage.encoder.named_parameters(),
                cnn_stage.encoder.named_parameters(),
            ):
                assert n1 == n2
    ...
    def test_attention_weights_loaded_correctly(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".pt") as f:
            ...
            for (n1, p1), (n2, p2) in zip(
                ...
                cnn_stage.attention.named_parameters(),
            ):
                assert n1 == n2
                torch.testing.assert_close(p1, p2), f"attention.{n1} 가중치 불일치"

    def test_local_map_decoder_weights_loaded_correctly(self):
        """체크포인트의 local_map_decoder 가중치가 정확히 로드된다."""
        cfg = _small_cfg()
        with tempfile.NamedTemporaryFile(suffix=".pt") as f:
            lm_stage = _save_local_map_ckpt(cfg, Path(f.name))
            cnn_stage = _make_cnn_stage(cfg)
            ...
            for (n1, p1), (n2, p2) in zip(
                lm_stage.local_map_decoder.named_parameters(),
                cnn_stage.local_map_decoder.named_parameters(),
            ):
                assert n1 == n2
                torch.testing.assert_close(p1, p2), f"local_map_decoder.{n1} 가중치 불일치"

    def test_freeze_true_disables_upstream_grad(self):
        """freeze=True이면 encoder + attention + local_map_decoder가 requires_grad=False."""
        cfg = _small_cfg()
        with tempfile.NamedTemporaryFile(suffix=".pt") as f:
            _save_local_map_ckpt(cfg, Path(f.name))
            stage = _make_cnn_stage(cfg)
            load_local_map_weights(f.name, stage, freeze=True)
            ...
            for name, p in stage.local_map_decoder.named_parameters():
                assert not p.requires_grad, f"local_map_decoder.{name} requires_grad가 True"

    def test_cnn_refiner_unaffected(self):
        """로드 후 cnn_refiner 파라미터는 requires_grad=True를 유지해야 한다."""
        cfg = _small_cfg()
        with tempfile.NamedTemporaryFile(suffix=".pt") as f:
            _save_local_map_ckpt(cfg, Path(f.name))
            stage = _make_cnn_stage(cfg)
            ...
            for name, p in stage.cnn_refiner.named_parameters():
                assert p.requires_grad, f"cnn_refiner.{name} grad 꺼짐"
...
class TestTrainEpochCNN:

    def _setup(self):
        cfg = _small_cfg()
        stage = _make_cnn_stage(cfg)
        # encoder + attention + local_map_decoder 동결
    ...
    def test_cnn_refiner_params_updated(self):
        """train_epoch 이후 cnn_refiner 파라미터가 변해야 한다."""
        stage, opt, cfg, loader = self._setup()
        before = {
            n: p.clone()
            for n, p in stage.cnn_refiner.named_parameters()
        }
        ...
        assert changed, "cnn_refiner 파라미터가 업데이트되지 않음"

    def test_frozen_upstream_unchanged(self):
        """동결된 encoder + attention + local_map_decoder 파라미터는 변하지 않아야 한다."""
        stage, opt, cfg, loader = self._setup()
        ...
        lmd_before  = {n: p.clone() for n, p in stage.local_map_decoder.named_parameters()}
        ...
        for n, p in stage.local_map_decoder.named_parameters():
            assert torch.equal(lmd_before[n], p), f"local_map_decoder.{n} 값이 변했음"
